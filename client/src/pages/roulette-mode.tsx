...
import { Sparkles, Flame, Brain, Settings2 } from "lucide-react";
...
export default function RouletteMode() {
  ...
  const [currentMode, setCurrentMode] = useState<GameMode>("classic"); // Added
  const { play } = useSound();
  ...
  return (
    <GameLayout title="">
      <div className="max-w-md mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-4xl"> {/* Game Mode Selection */}
              ...
              {selectedPlayer && !isSelecting && (
                ...
                >
                  ...
                  <p className="text-white/80 text-xl mb-6">
                    {modes[currentMode].options.length > 0 ? modes[currentMode].options[Math.floor(Math.random()*modes[currentMode].options.length)].text : `Beba ${numDrinks} ${numDrinks === 1 ? 'gole' : 'goles'}!`}
                  </p>

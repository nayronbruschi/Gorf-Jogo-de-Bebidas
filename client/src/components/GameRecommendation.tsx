...
export function GameRecommendationCard({ className }: GameRecommendationCardProps) {
  ...
  if (!recommendation) {
    return (
      <Card className={className}>
        <CardHeader>
          <CardTitle className="text-lg">Jogo Recomendado</CardTitle>
          <CardDescription>Complete seu perfil para receber recomendações</CardDescription>
        </CardHeader>
  ...
  return (
    <Card className={`${className} overflow-hidden`}>
      <CardHeader className="bg-gradient-to-r from-purple-600 to-blue-500 text-white pb-3">
      ...
      <CardContent className="pt-4">
        <div className="space-y-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1 line-clamp-2">{recommendation.description}</p>
          </div>
          
          {recommendation.reasonsToPlay.length > 0 && (
            <div className="flex items-start">
              <ThumbsUp className="h-4 w-4 mr-2 text-green-500 flex-shrink-0 mt-0.5" />
              <span className="text-sm">{recommendation.reasonsToPlay[0]}</span>
            </div>
          )}
